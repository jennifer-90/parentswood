...
const updatePassword = () => {
    form.patch(route('profile.updatePassword'), {
        preserveScroll: true,
        onSuccess: () => form.reset(), // on vide tout en cas de succès
        onError: () => {
            // on vide seulement les champs sensibles concernés
...
// (Optionnel) indicateur très simple de force du mot de passe
const strengthInfo = computed(() => {
    ...
    if (/\W/.test(pwd)) score++
    const map = [
        { label: 'Faible', width: '25%' },
        { label: 'Moyen', width: '50%' },
        { label: 'Bon', width: '75%' },
        { label: 'Fort', width: '100%' },
    ]
    return score ? map[score - 1] : { label: 'Très faible', width: '10%' }
})
</script>
        ...
        <div class="space-y-6">
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs font-medium text-gray-500">{{ strengthInfo.label }}</span>
                </div>
                <div class="w-full bg-gray-200 rounded-full h-2">
                    <div class="bg-yellow-400 h-2 rounded-full" :style="{ width: strengthInfo.width }"></div>
                </div>
            </div>
